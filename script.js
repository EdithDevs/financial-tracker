...
function updateValues() {
  ...
  balance.textContent = `GH${total}`;
  income.textContent = `GH${incomeTotal}`;
  expense.textContent = `GH${expenseTotal}`;
}
...
function addTransactionDOM(transaction) {
  ...
  tr.innerHTML = `
    ...
    <td class="amount ${transaction.amount < 0 ? 'expense' : ''}">GH${transaction.amount.toFixed(2)}</td>
